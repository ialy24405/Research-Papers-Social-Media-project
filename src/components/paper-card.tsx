...
import { Bookmark, Download, Heart, MessageCircle, MoreVertical, Share2 } from 'lucide-react';
...
export function PaperCard({ paper }: PaperCardProps) {
  ...
  useEffect(() => {
    setTimeAgo(formatDistanceToNow(new Date(paper.createdAt), { addSuffix: true }));
  }, [paper.createdAt]);
    ...
    <Card className="flex flex-col">
      <CardHeader>
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Avatar className="h-6 w-6">
            <AvatarImage src={paper.authorAvatar} alt={paper.authorName} />
      ...
      <CardFooter className="flex-col items-start gap-4">
        <div className="flex items-center flex-wrap gap-4 text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
            <Share2 className="h-4 w-4" /> {paper.interactions.shares}
          </div>
        </div>
