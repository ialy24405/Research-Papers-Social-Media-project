...
export default async function handler(
	...
	try {
		...
		// If rejected and reason provided, could store in a separate table
		// For now, just log the reason
		if (status === "rejected" && reason) {
			console.log(`Paper ${paperId} rejected with reason: ${reason}`);
		}

		res.status(200).json({ message: `Paper status updated to ${status}` });
	} catch (error) {
